CLASS zcl_generate_travel_data_RDA DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_travel_data_RDA IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:itab TYPE TABLE OF ztravel_RDA.
...
*   delete existing entries in the database table
    DELETE FROM ztravel_RDA.

*   insert the new table entries
    INSERT ztravel_RDA FROM TABLE @itab.

*   check the result
    SELECT * FROM ztravel_RDA INTO TABLE @itab.
    out->write( sy-dbcnt ).
