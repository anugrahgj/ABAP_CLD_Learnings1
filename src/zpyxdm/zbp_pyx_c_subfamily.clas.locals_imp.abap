CLASS lhc_ZPYX_C_SUBFAMILY DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS augment FOR MODIFY
      IMPORTING entities_create FOR CREATE Subfamily
                entities_update FOR UPDATE Subfamily.

ENDCLASS.

CLASS lhc_ZPYX_C_SUBFAMILY IMPLEMENTATION.

  METHOD augment.
    DATA: lt_text_fornew TYPE TABLE FOR CREATE zpyx_i_subfamily\_Text,
          lt_text_forcba TYPE TABLE FOR CREATE zpyx_i_subfamily\_Text,
          lt_text_forupd TYPE TABLE FOR UPDATE zpyx_i_subfamily_t.

    DATA: ls_text_tky_link TYPE STRUCTURE FOR READ LINK zpyx_i_subfamily\_Text,
...
*   Create new text for new Family
    ...
    MODIFY AUGMENTING ENTITIES OF zpyx_i_subfamily
      ENTITY Subfamily
        CREATE BY \_Text
        FROM lt_text_fornew
    ...
    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF zpyx_i_Subfamily
       ENTITY subfamily BY \_Text
