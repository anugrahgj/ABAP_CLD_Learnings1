CLASS lcl_handler DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PUBLIC SECTION.
    CLASS-DATA:
      mt_root_to_create TYPE STANDARD TABLE OF zroom_jbp WITH NON-UNIQUE DEFAULT KEY,
      mt_root_to_update TYPE STANDARD TABLE OF zroom_jbp WITH NON-UNIQUE DEFAULT KEY,
      mt_root_to_delete TYPE STANDARD TABLE OF zroom_jbp WITH NON-UNIQUE DEFAULT KEY.
  PRIVATE SECTION.
    TYPES:
      ty_action(2)     TYPE c,
      tt_message       TYPE TABLE OF symsg,
      tt_room_failed   TYPE TABLE FOR FAILED z_i_room_jbp,
      tt_room_reported TYPE TABLE FOR REPORTED z_i_room_jbp.
    CONSTANTS:
      BEGIN OF cs_action,
...
CLASS lcl_handler IMPLEMENTATION.
  ...
  METHOD create.
    LOOP AT roots_to_create INTO DATA(ls_root_to_create).
      DATA(lv_message) = validate( iv_action = cs_action-create is_room = CORRESPONDING zroom_jbp( ls_root_to_create ) ).
  ...
  METHOD update.
    LOOP AT roots_to_update INTO DATA(ls_root_to_update).
      SELECT SINGLE FROM zroom_jbp FIELDS * WHERE id = @ls_root_to_update-id INTO @DATA(ls_room).
  ...
  METHOD delete.
    LOOP AT roots_to_delete INTO DATA(ls_root_to_delete).
      SELECT SINGLE FROM zroom_jbp FIELDS * WHERE id = @ls_root_to_delete-id INTO @DATA(ls_room).
    ...
    LOOP AT roots_to_lock INTO DATA(ls_root_to_lock).
      TRY.
          cl_abap_lock_object_factory=>get_instance( iv_name = 'EZROOMXXX')->enqueue(
            it_table_mode = VALUE if_abap_lock_object=>tt_table_mode( ( table_name = 'ZROOM_jbp' ) )
  ...
  METHOD read.
    LOOP AT roots_to_read INTO DATA(ls_root_to_read).
      SELECT SINGLE FROM z_i_room_jbp FIELDS * WHERE id = @ls_root_to_read-id INTO @DATA(ls_room).
      INSERT ls_room INTO TABLE et_room.
    ENDLOOP.
  ENDMETHOD.

  METHOD validate.
    AUTHORITY-CHECK OBJECT 'Z_LOCAOJBP' ID 'ACTVT' FIELD iv_action ID 'Z_LOCAFJBP' FIELD is_room-location.
    IF sy-subrc <> 0.
...
CLASS lcl_saver IMPLEMENTATION.
  ...
  METHOD save.
    INSERT zroom_jbp FROM TABLE @lcl_handler=>mt_root_to_create.
    UPDATE zroom_jbp FROM TABLE @lcl_handler=>mt_root_to_update.
    DELETE zroom_jbp FROM TABLE @lcl_handler=>mt_root_to_delete.
  ENDMETHOD.
ENDCLASS.
