CLASS zcl_btp_file_uploader_copy DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    DATA: tab_filedata TYPE STANDARD TABLE OF string.
    METHODS: get_input_field_value IMPORTING i_name         TYPE string
                                             i_dataref      TYPE data
                                   RETURNING VALUE(o_value) TYPE string.
    METHODS: get_html RETURNING VALUE(ui_html) TYPE string.
ENDCLASS.



CLASS zcl_btp_file_uploader_copy IMPLEMENTATION.
...
* extract the filename and fileext from this metadata as well as the raw file data.
        ...
        DELETE content FROM ( lines( content ) - 12 ) TO lines( content ).  " get rid of the last 12 lines

        LOOP AT content REFERENCE INTO content_item.  " put it all back together again humpdy dumpdy....
          filedata =  content_item->*.
          APPEND filedata TO tab_filedata.
          CLEAR filedata.
        ENDLOOP.
...
* Unpack input field values such as tablename, dataoption, etc.
        ...
        IF ui_dataref IS BOUND.
          ...
          tablename = me->get_input_field_value( i_name = `TABLENAME` i_dataref = <ui_dataref> ).
          dataoption = me->get_input_field_value( i_name = `DATAOPTION` i_dataref = <ui_dataref> ).
        ENDIF.
...
*        IF fileext <> `json`.
* Check file extension is valid, only csv can upload.
        IF fileext <> `csv`.
          response->set_status( i_code = if_web_http_status=>bad_request
...
* Load the data to the table via dynamic internal table
        ...
        FIELD-SYMBOLS: <rec_table_structure> TYPE any.
        ...
        TRY.
            ...
            CREATE DATA lo_line LIKE LINE OF <table_structure>.
            ASSIGN lo_line->* TO <rec_table_structure>.
...
*                                   CHANGING data = <table_structure> ).
        DATA: tab_items   TYPE STANDARD TABLE OF string,
              rec_items   TYPE string,
              wrk_yyyy(4) TYPE n,
              wrk_mm(2)   TYPE n,
              wrk_dd(2)   TYPE n,
              wrk_index   TYPE i.
        FIELD-SYMBOLS: <l_fs_value> TYPE any.
        "edit csv file to table data
        LOOP AT tab_filedata INTO filedata.
          " skip the header line 1
          IF sy-tabix = 1
          OR filedata IS INITIAL
          OR filedata CO ','.
            CONTINUE.
          ENDIF.

          SPLIT filedata AT ',' INTO TABLE  tab_items.
          LOOP AT tab_items INTO rec_items .
            wrk_index = sy-tabix + 1.
            " skip key client
            ASSIGN COMPONENT wrk_index OF STRUCTURE <rec_table_structure>
               TO <l_fs_value>.
            "Change date's format(delete '/')
            IF rec_items CO '/0123456789'.
              SPLIT rec_items AT  '/' INTO wrk_yyyy wrk_mm wrk_dd.
              IF wrk_mm <= '9'.
                CONCATENATE '0' wrk_mm INTO wrk_mm.
              ENDIF.
              IF wrk_dd <= '9'.
                CONCATENATE '0' wrk_dd INTO wrk_dd.
              ENDIF.
              CLEAR rec_items.
              CONCATENATE wrk_yyyy wrk_mm wrk_dd INTO rec_items.
            ENDIF.

            <l_fs_value>   = rec_items .
          ENDLOOP.
          APPEND <rec_table_structure> TO <table_structure>.
        ENDLOOP.
  ...
  METHOD get_input_field_value.
    ...
    ASSIGN COMPONENT i_name  OF STRUCTURE i_dataref TO <l_field>.
    IF <l_field> IS ASSIGNED.
      ASSIGN <l_field>->* TO <l_value>.
      o_value = condense( <l_value> ).
    ENDIF.
  ...
  METHOD get_html.
    ...
    |<!DOCTYPE HTML> \n| &&
     ...
     |        data-sap-ui-theme="sap_fiori_3" data-sap-ui-xx-bindingSyntax="complex" data-sap-ui-compatVersion="edge" \n| &&
*     |        data-sap-ui-theme="sap_fiori_3_dark" data-sap-ui-xx-bindingSyntax="complex" data-sap-ui-compatVersion="edge" \n| &&
     ...
     |                shell.setTitle("BTP ABAP File Uploader") \n| &&
