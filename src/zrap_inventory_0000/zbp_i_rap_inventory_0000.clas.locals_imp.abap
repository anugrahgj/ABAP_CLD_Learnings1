...
CLASS LHC_INVENTORY IMPLEMENTATION.
  METHOD CALCULATEINVENTORYID.
    "Ensure idempotence
    READ ENTITIES OF zi_rap_inventory_0000 IN LOCAL MODE
      ENTITY Inventory
        FIELDS ( InventoryID )
    ...
    "Get max travelID
    SELECT SINGLE FROM zrap_inven_0000 FIELDS MAX( inventory_id ) INTO @DATA(max_inventory).

    "update involved instances
    MODIFY ENTITIES OF zi_rap_inventory_0000 IN LOCAL MODE
      ENTITY Inventory
  ...
  METHOD GetPrice.
   DATA destination  TYPE REF TO if_soap_destination.
    DATA proxy TYPE REF TO zrap_0000_co_epm_product_soap .
    ...
    LOOP AT inventories ASSIGNING FIELD-SYMBOL(<inventory>).
      ...
      TRY.
          ...
          ENDIF.
          IF proxy IS INITIAL.
            proxy = NEW zrap_0000_co_epm_product_soap(
                             destination = destination
                           ).
          ENDIF.

          DATA(request) = VALUE zrap_0000_req_msg_type( req_msg_type-product = <inventory>-ProductID ).
          proxy->get_price(
            EXPORTING
              input = request
            IMPORTING
        ...
        CATCH zrap_0000_cx_fault_msg_type INTO DATA(soap_exception).
    ...
    IF reported_inventory_soap IS NOT INITIAL.
